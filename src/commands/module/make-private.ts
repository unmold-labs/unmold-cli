...
export default class ModuleMakePrivate extends Command {
  static override description = "Make a published module version private";
  ...
  public async run(): Promise<void> {
    const { args, flags } = await this.parse(ModuleMakePrivate);

    try {
      const userProfile = await getUserProfile();
      const metadata: IModuleMetadata & { access: "private" } = {
      ...
      this.log(
        `✅ Successfully made ${metadata.namespace}/${metadata.name}/${metadata.system}@${metadata.version} private`,
      );
    } catch (error) {
      if (error instanceof Error) {
        this.error(`Failed to make module private: ${error.message}`, {
          exit: 1,
        });
      } else {
