...
describe('list', () => {
  it('should print module versions with the default system name', async () => {
    const scope = nock(config.api.url)
      .get('/v1/modules/test-ns/test-mod/generic/versions')
      .reply(200, {
        modules: [
          {
            versions: [
  ...
  it('should print module versions with the specified system name', async () => {
    const scope = nock(config.api.url)
      .get('/v1/modules/test-ns/test-mod/aws/versions')
      .reply(200, {
        modules: [
          {
            versions: [
    ...
    const {stdout} = await runCommand(['module', 'list', 'test-ns', 'test-mod', '--system', 'aws'])
    expect(JSON.parse(stdout)).to.deep.equal([
      {
        namespace: 'test-ns',
        name: 'test-mod',
